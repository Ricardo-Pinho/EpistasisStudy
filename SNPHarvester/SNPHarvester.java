...
public class SNPHarvester extends BasicAlg{
				    ...
				    for(int i=0; i<colCount-1; i++) {
				    	...
				    	if(indicator[i]==0 && testSingleSNPFeasibility(i) &&
				          ...
				          if(tmp>0){
				        	  ...
				        	  String str = orginalValue+" (";
				        	  for(int h=0;h<k;h++){
				        		  str = str + selectedSNPSet[h]+",";
				        	  }
	 ...
	 * 
	 */
	public ArrayList thresholdBasedRunningProcess
	...
	public double getObjectiveValue(int[] selectedSNPSet) throws Exception{
     ...
     /* Obtain the changed interaction measure when swap happens*/
      ...
      /* An example on how to run SNPHarvester */
     public static void main(String[] args) throws Exception{
    	 
    	 String path ="WTCC_chr22.txt";
		 ...
		 if(resultType.equals("thresholdBased")){
			 result = alg.thresholdBasedRunningProcess(path,measure,pValue,numSuccessiveRun,mink,maxk);
		 }
		 
		 if(resultType.equals("topKBased")){
			 result = alg.topKBasedRunningProcess(path,measure,topK,numSuccessiveRun,mink,maxk);
		 }
		 ...
		 System.out.println("Runnning Time:"+(end-start)/(double)1000);
