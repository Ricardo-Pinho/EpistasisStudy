...
newfile2="$3/$(basename $(dirname ${file}))"

temp=$(echo "$file" | sed -e 's/.*1750.1.//g')
ind=$( echo "$temp" | sed -e 's/-3GE-.*//g' )

if  [[ ! -f "$newfile2/results${ind}.txt" ]] && [[ ! -f "${newfile}.txt" ]] 
then
