...
int NUM_OF_ONES_IN_Y=MAX_NUM_OF_INDIVIDUALS/2;
int MAX_NUM_OF_SNPS=2;
...
void calculate_snps_diff(vector<SNP>& X, list<edge> & mst, map< pair<int,int>, SNPs_Diff> & snps_diff){
    //SNPs_Diff dst_diff;
    int k=0;
...
int calculate_test_scores(Data_map & data_map,
                          ...
                          int **abcdef,__int64 *bins, double & chi_alpha){
	
	for (int ind=0 ; ind<NUM_OF_PERMUTATIONS ; ++ind){
...
void traverse_mst(int root, int new_root, vector< set<edge> > &vertex_edges,
				  ...
				  int p, int q, int o, int g, int h, int l,int **abcdef,double &chi_alpha,
                  __int64 &ideal_weight,clock_t &tt_traverse_tree,clock_t &tt_update_old,clock_t &tt_cal_bounds,__int64 &old_update_times, __int64 *bins){
...
//			}		
        ...
        real_mst_weight+=(temp->weight*(jj+1));
...
/*Initialize updating seed*/
		...
		tt_temp=clock();		
		calculate_test_scores(data_map,a2_y,a3_y,b2_y,b3_y,e2_y,e3_y,s,t,r,p,q,o,g,h,l,abcdef,bins,chi_alpha);
...
//		if (previous_node==-1)
			...
			traverse_mst(root,check_vertex,vertex_edges,snps_diff,data_map,X,
						 ...
						 previous_plus,
						 p,q,o,g,h,l,abcdef,chi_alpha,
                         ideal_weight,tt_traverse_tree,tt_update_old,tt_cal_bounds,old_update_times,bins);
...
*/
            ...
            __int64 job_percent=(((jj-from+1)*1000/snps_range_size));
            
            if ((job_percent-o_job_percent)>=(1)){
                o_job_percent=job_percent;
                cout <<"[" <<id <<"] " <<setw(5) <<job_percent/10.0 <<"% : " <<chi_alpha <<endl;                
            }            
	}
/*    
    if ((jj+1)%100) {
...
*/    
	...
	cout <<"[" <<id <<"] " <<"Real mst weight:" <<real_mst_weight <<endl;
	cout <<"[" <<id <<"] " <<"(B) Percentage:" <<(real_mst_weight*100.0/size)/MAX_NUM_OF_INDIVIDUALS <<"%" <<endl;
	...
	*/
    ...
    if (bins_fn==NULL){        
        ...
        for (int i=0 ; i<n_bins ; ++i){
            cout <<id <<"\t" <<i*bin_interval <<"\t" <<bins[i] <<endl;
        }
    }
    else {        
        char fn_suffix[10];
        ...
        for (int i=0 ; i<n_bins ; ++i){
            bins_output <<i*bin_interval <<"\t" <<bins[i] <<endl;
        }
        bins_output.close();        
    }
    delete []bins;
...
int main(int argc, char * argv[]) {
    ...
    //*Read Data*//
    read_genotypes(geno_infile,X);    
    Phenotype Y_origin=read_phenotypes(pheno_infile);
	...
	// cout <<"OK" <<endl;
	// cout <<Ylist.size() <<" permutations generated." <<endl;	
    ...
    __int64 progress,o_progress=0;
	ii=0;
	for (vector<SNP>::iterator iter1=X.begin(); iter1!=X.end() ; ++iter1, ++ii){
		jj=0;		
		for (list <Phenotype>::iterator iter2=Ylist.begin(); iter2!=Ylist.end() ; ++iter2, ++jj){
