...
double calculate_test_scores(Data_map & data_map,
...
*/                    
				...
				*/
				...
				ttt_temp=clock();
                double tmp_score=calculate_test_scores(data_map,
                ...
                if (tmp_score>=score_threshold){
                    cout <<"SNP-pair (" <<root <<"," <<other_vertex <<"):" <<tmp_score <<endl;
                }
...
*/
    ...
    cout <<"[" <<id <<"] " <<"Parallel SNP range:" <<from <<"--" <<to <<"\t: " <<size <<" pairs" <<endl <<endl;

    cout <<"======Significant SNP-pairs=====" <<endl;
...
*/			
		...
		tt_temp=clock();
		double tmp_score=calculate_test_scores(data_map,a2_y,a3_y,b2_y,b3_y,e2_y,e3_y,s,t,r,p,q,o,g,h,l,abcdef,bins,org_bins,chi_alpha,chi_alpha_array);
        
        if (tmp_score>=score_threshold){
            cout <<"SNP-pair (" <<root <<"," <<check_vertex <<"):" <<tmp_score <<endl;
        }
...
int main(int argc, char * argv[]) {
    ...
    */
    NUM_OF_PERMUTATIONS=1; // Consider the original phenotypes a permutation

    score_threshold=read_qvalue_list(qvalue_infile,qvalue_threshold);
 
     //cout <<test_score <<endl;
     if (score_threshold<0) {
         cout <<"No SNP-pair is significant!" <<endl;
         exit(0);
     }
     else {
         cout <<"Significant SNP-pair(s) found!" <<endl;
         cout <<"Test value Threshold:" <<score_threshold <<endl;
     }
